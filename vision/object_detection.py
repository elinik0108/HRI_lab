...
from HRI_lab_Pepper.config import B, W, R, YOLO_DETECT_MODEL, YOLO_PT_MODEL, YOLO_ONNX_MODEL, INFERENCE_DEVICE
...
class ObjectDetector:
    ...
    def _try_init_ort(self, onnx_model_path: str) -> "str | None":
        ...
        try:
            ...
            print(f"{B}[ObjectDetector] Loading {onnx_model_path} via onnxruntime-gpu …{W}")
            providers = ["CUDAExecutionProvider", "CPUExecutionProvider"]
            self._ort_session    = ort.InferenceSession(onnx_model_path, providers=providers)
            self._ort_input_name = self._ort_session.get_inputs()[0].name
            print(f"{B}[ObjectDetector] Ready — ONNX Runtime GPU{W}")
            return "ort_gpu"
        except ImportError:
            print(f"{R}[ObjectDetector] onnxruntime not installed — falling back to OpenVINO CPU.{W}")
            return None
        except Exception as exc:
            print(f"{R}[ObjectDetector] ORT GPU init failed ({exc}) — falling back to OpenVINO CPU.{W}")
            return None
